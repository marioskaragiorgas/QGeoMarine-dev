...
from datahandling import SEISMIC, DatabaseManager, MAGGY, ExportData
...
class FileParseWorker(QThread):
    ...
    def __init__(self, file_path):
        ...
        self.segy_handler = SEISMIC.SEGYHandler(db_file_path=self.file_path, bin_file_path=None)
...
class SeismicEditor(QMainWindow):
    ...
    def __init__(self, seismic_filepath, db_file_path, parent = None):
        ...
        self.segy_handler = SEISMIC.SEGYHandler(db_file_path=db_file_path, bin_file_path=None)
    ...
    def on_parsing_finished(self, result):
        ...
        file_item.setText(0, os.path.basename(self.db_file_path))
    ...
    def export_file(self):
        ...
        if file_extension in ['.sgy', '.segy']:
            export = ExportData(data, file_path, self.db_file_path)
            export.export_segy()
            ...
            export = ExportData(data, file_path, self.db_file_path)
            export.export_image(delta=self.sample_interval*1e3, filename= os.path.splitext(os.path.basename(file_path)))[0]        
        else:
    ...
    def import_wavelet(self):
        ...
        if file_path:
            try:
                data = MAGGY.CSV_TXT_XLS.load_files(self, file_path)
